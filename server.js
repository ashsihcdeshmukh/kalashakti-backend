...
app.post('/api/auth/verify-otp', (req, res) => {
  try {
    ...
    return res.json({
      ...
      token: `kalashakti_token_${Date.now()}`,
      student: student
    });
  } catch (err) {
...
// 11. Central Server Synchronization Endpoints (Cross-Device 2-Way Sync)
...
app.get('/api/sync/:table', (req, res) => {
  try {
    ...
    const db = store.readDb();
    let data = [];
    if (table === 'cms' || table === 'app-content') {
      data = db.cms || db.studioCMS || null;
    } else {
      data = db[table] || [];
    }
...
app.post('/api/sync/:table', (req, res) => {
  try {
    ...
    if (table === 'cms' || table === 'app-content') {
      db.cms = payload;
      db.studioCMS = payload;
      store.writeDb(db);
      console.log(`[SYNC] Live CMS content updated centrally on server`);
      return res.json({ success: true, table, updated: true });
    }

    if (Array.isArray(payload)) {
      // Overwrite / sync full table array
      db[table] = payload;
    } else if (payload && typeof payload === 'object') {
      // Upsert single record
      if (!Array.isArray(db[table])) db[table] = [];
      const id = payload.id;
      const idx = id ? db[table].findIndex(item => item.id === id) : -1;
      if (idx >= 0) {
    ...
    store.writeDb(db);
    console.log(`[SYNC] Synced table '${table}' centrally: ${Array.isArray(db[table]) ? db[table].length : 1} records`);
    res.json({
      success: true,
      table,
      count: Array.isArray(db[table]) ? db[table].length : 1
    });
  } catch (err) {
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', studio: 'Kalashakti Group of Academies (कला शक्ति)', time: new Date().toISOString() });
});

// Start Server
app.listen(PORT, () => {
  console.log('====================================================');
  console.log(`💃 KALASHAKTI GROUP OF ACADEMIES (कला शक्ति) SERVER`);
