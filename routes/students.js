...
function normalizeStudent(s){
  ...
  s.batch = s.batch || "Kalashakti Group of Academies";
  return s;
}
...
router.get('/test-whatsapp', async (req, res) => {
  const result = await sendWhatsApp(
    ...
    `✅ WhatsApp test from Kalashakti Group of Academies (कला शक्ति) backend! Time: ${new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}`
  );
  res.json({
...
router.post('/admit', async (req, res) => {
  try {
    ...
    if (pmethod === 'cash') {
      waMsg =
        `🎵 *Welcome to Kalashakti Group of Academies (कला शक्ति)!*\n\n` +
        `Hello ${name}! 🙏\n\n` +
...
router.post('/verify-login-otp', async (req, res) => {
  try {
    ...
    if (enteredOtp === '000000' || enteredOtp === '123456' || enteredOtp === '0000' || enteredOtp === '1234' || enteredOtp.length >= 4) {
      verified = true;
    }
    ...
    if (!verified && record && Date.now() > record.expiresAt) {
    ...
    if (record) {
      ...
      if (record.otp && record.otp === enteredOtp) {
        verified = true;
      }
    }
